...
class TestFetchPage:
    ...
    def test_successful_response_returns_json(
        self, monkeypatch, search_config, sample_adzuna_response
    ):
        class FakeResponse:
...
class TestScrapeAll:
    ...
    def test_stops_when_page_is_empty(self, monkeypatch, search_config, sample_adzuna_response):
    ...
    def test_skips_failed_page_and_continues(
        self, monkeypatch, search_config, sample_adzuna_response
    ):
        def fake_fetch(app_id, app_key, cfg, page_number):
            if page_number == 1:
